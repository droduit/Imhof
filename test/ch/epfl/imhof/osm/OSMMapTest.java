...
public class OSMMapTest {
    ...
    public void waysListNonModifiable() throws Exception {
        ...
        /*
         * There are two ways to achieve immutability of this classs: 1. Use
         * unmodifiable collections 2. Make deepCopy in constructor and in
         * getters
         */
        ...
        if (!IM_COLL_WAYS) {
            ...
            // Check deep copy is done in the gettter. We can assume deep copy
            // is done in the constructor. If not test will fail
    ...
    public void relationsListNonModifiable() throws Exception {
        ...
        testRelation1Members.add(new OSMRelation.Member(
                OSMRelation.Member.Type.NODE, "First", testNode1));
        testRelation1Members.add(new OSMRelation.Member(
                OSMRelation.Member.Type.NODE, "Second", testNode2));
        testRelation1Members.add(new OSMRelation.Member(
                OSMRelation.Member.Type.WAY, "Total", testWay1));

        HashMap<String, String> testData6 = sampleAttributesValues();
        Attributes testAttributes6 = new Attributes(testData6);
        OSMRelation testRelation1 = new OSMRelation(6, testRelation1Members,
                testAttributes6);
        testRelationList.add(testRelation1);

        List<OSMRelation.Member> testRelation2Members = new ArrayList<>();
        testRelation2Members.add(new OSMRelation.Member(
                OSMRelation.Member.Type.NODE, "First", testNode2));
        testRelation2Members.add(new OSMRelation.Member(
                OSMRelation.Member.Type.NODE, "Second", testNode3));
        testRelation2Members.add(new OSMRelation.Member(
                OSMRelation.Member.Type.WAY, "Total", testWay2));

        HashMap<String, String> testData7 = sampleAttributesValues();
        Attributes testAttributes7 = new Attributes(testData7);
        OSMRelation testRelation2 = new OSMRelation(7, testRelation2Members,
                testAttributes7);
        testRelationList.add(testRelation2);

        OSMMap testMap = new OSMMap(testWayList, testRelationList);
        OSMRelation relationToAdd = new OSMRelation(8, testRelation2Members,
                testAttributes7);
        ;
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (t.toString().equals(
                    "java.util.List<ch.epfl.imhof.osm.OSMRelation>")) {
                field = fields[i];
                count++;
            }
        }
        ...
        field.setAccessible(true);
        List<OSMRelation> relationsList = (List<OSMRelation>) field
                .get(testMap);
        ...
        if (!IM_COL_RELATIONS) {
            ...
            constructorDeepCopy = testMap2.relations().size() != testRelationList
                    .size();

            // Check deep copy is done in the getter. We can assume deep copy is
            // done in the constructor. If not test will fail
    ...
    public void testGetterWays() {
        ...
        if (DEEP_COPY_WAYS) {
            // deep copy in getter should be done when not using immutable
            // collections
            assertFalse(map.ways() == map.ways());
        } else {
            if (IM_COLL_WAYS) {
                // deep copy in getter should not be done when using immutable
                // collections
    ...
    public void testAddRelation() {
        OSMMap.Builder builder = new OSMMap.Builder();
        long id = 5;
        OSMRelation relation = new OSMRelation(id, new ArrayList<Member>(),
                new Attributes(sampleAttributesValues()));
    ...
    public void testBuild() {
        ...
        long idRelation = 5;
        OSMRelation relation = new OSMRelation(idRelation,
                new ArrayList<Member>(), new Attributes(
                        sampleAttributesValues()));
        builder.addRelation(relation);
